...
let DashboardService = class DashboardService {
    ...
    async getDashboardData(query) {
        const [quickStats, monthlyRevenue, serviceStats, walletBalances, recentTransactions, serviceIncome,] = await Promise.all([
    ...
    async getQuickStats(query) {
        ...
        const todaysCollection = todayTransactions.reduce((sum, t) => sum + t.amount, 0);
        const todaysProfit = todayTransactions.reduce((sum, t) => sum + t.profit, 0);
    ...
    async getMonthlyRevenue(query) {
        ...
        for (let month = 0; month < 12; month++) {
            ...
            monthlyData.push(monthRevenue._sum.amount || 0);
        }
        return {
    ...
    async getIncomeBreakdown(query) {
        ...
        const series = details.map((item) => Math.round((item.value / total) * 100));
    ...
    getTransactionDisplayName(transaction) {
        ...
        return (nameMap[transaction.subType || ''] || transaction.type.replace('_', ' '));
    }
};
